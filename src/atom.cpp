...
void Atom::init() {
   ...
   if (Atom::buckets == nullptr) {
	   Atom::buckets = (Atom**)malloc(sizeof(Atom*) * ATOM_BUCKETS_SIZE);
	   for (int i = 0; i < ATOM_BUCKETS_SIZE; i++) {
			Atom::buckets[i] = nullptr;
	   }
   }
   ...
   for(h = 0, i=0; i< this->length; i++) {
       h = (h<<1) + scatter[(unsigned char)this->str[i]];
   }

   //h %= NELEMS(buckets);
   h %= ATOM_BUCKETS_SIZE;
   printf("scatter and hash, then h is %ul\n", h);

   p = buckets[h];
   if (p != NULL) {
	   for (; p; ) {
		   if (this->length == p->length) {
			   for (i = 0; i < this->length && p->str[i] == this->str[i];) {
				   i++;
			   }
			   if (i == this->length) {
    ...
    //p = (atom *) malloc(sizeof((atom)(*p)) + len + 1);
    //p->length = len;
    //p->str = (char *)(p + 1);
    //if(len > 0) {
...
Atom::Atom(long n) {
	...
	this->str = (char*)malloc(sizeof(char) * (this->length + 1));
	memcpy(this->str, s, this->length + 1);
	init();
}